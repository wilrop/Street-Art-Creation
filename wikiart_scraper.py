# Code taken from: https://github.com/robbiebarrat/art-DCGAN/blob/master/genre-scraper.py
# Adapted to run single threaded
import time
import os
import re
import random
...
parser.add_argument("--num_pages", type=int, help="number of pages to scrape (leave blank to download all of them)",
                    default=1000)
...
def main(typep, searchword, num_pages, output_dir):
    ...
    for i in numbers:
        page = get_painting_list(i, typep, searchword)
        wikiart_pages.append(page)
        if (i + 1) % 10 == 0:
            print("Gathered links from " + str(i + 1) + " pages")
